...
Class CRUD_Model extends CI_Model
{
	...
	*/
	...
	public function gets( $args = array() ){
		...
		if( $left_outer_join ){
			foreach ($left_outer_join as $join_key => $join_value) {
				$this->db->join($join_key, $join_value, 'left outer');
			}
		}
	...
	public function get( $args ){
		if( is_array($args) ){
			if( $data = $this->gets($args) ){
				return $this->gets($args)[0];
			}
			
			return false;
		}else{
			if ($data = $this->gets(array('where' => array($this->primary_key => $args)))) {
				return $data[0];
			}
	...
	public function gets_view($args = array() ){
		$args = array_merge($this->view, $args);
	...
	public function get_view($args = array() ){
		if (is_array($args)) {
			...
			if(  $data = $this->gets_view($args) ){
				return $data[0];
			}
			
			return false;
		}else{
			if ($data = $this->gets_view(array('where' => array($this->primary_key => $args)))) {
				return $data[0];
			}
